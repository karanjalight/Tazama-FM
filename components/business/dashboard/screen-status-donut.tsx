...
export interface DashboardScreenStatus {
  total: number;
  online: number;
  offline: number;
  pending: number;
...
export interface DashboardOfflineScreen {
  name: string;
  location: string;
  since: string;
}
...
export function ScreenStatusDonut({
  ...
  offlineScreens: DashboardOfflineScreen[];
}) {
  const { total, online, offline, pending } = status;
  const segments = [
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      ...
      {total === 0 ? (
        <p className="mt-4 text-sm text-muted-foreground">No screens paired yet.</p>
      ) : (
        <>
          ...
          {offlineScreens.length > 0 && (
            <div className="mt-4 border-t border-border pt-4">
              ...
              {offlineScreens.map((screen) => (
                <div
                  key={`${screen.location}-${screen.name}`}
