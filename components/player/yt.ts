...
export type YTPlayer = {
   ...
   * the player is actually seekable), so a separate seekTo() issued right
   * after it races the cueing exactly like pauseVideo() races
   * loadVideoById(), and the seek silently gets dropped. Passing the start
   * position as part of the same cue command is the API's built-in,
   * race-free way to cue-at-a-position.
   */
